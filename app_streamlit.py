...
with tabs[1]:
    ...
    if turn_log:
        ...
        # Summary metrics — SESSION CUMULATIVE TOTALS across all turns
        ...
        st.caption(f"📋 **{num_turns} turn{'s' if num_turns != 1 else ''} recorded** — summary cards show session cumulative totals; table shows one row per turn.")

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("📥 Session Input Tokens",    f"{total_in:,}",   help="Total tokens sent to the LLM across all turns in this session")
        c2.metric("📤 Session Output Tokens",   f"{total_out:,}",  help="Total tokens generated by the LLM across all turns in this session")
        c3.metric("💰 Session Total Cost",      f"${total_cost:.4f}", help="Cumulative LLM cost for this session")
        c4.metric("⚡ Avg LLM Latency / Turn",  f"{avg_llm:.0f} ms", help="Average time the LLM took to respond, per turn")

        st.divider()

        # Full per-turn table — each row = one exchange in the conversation
        st.caption("👇 **Per-turn breakdown** — each row is one exchange (user message → agent response)")
        st.dataframe(
            df_turns,
            width="stretch",
            hide_index=True,
